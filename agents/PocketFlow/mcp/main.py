...
from utils import call_llm, initialize_mcp_clients, get_all_tools, call_mcp_tool, cleanup_mcp_clients, get_mcp_client
import yaml
import sys
import asyncio
import logging
...
class InitializeMCPNode(AsyncNode):
    """Initialize connections to all MCP servers."""
    
    async def prep_async(self, shared):
        """Prepare for MCP initialization"""
    ...
    async def exec_async(self, config_path):
        ...
        try:
            multi_client = await initialize_mcp_clients(config_path)
            return multi_client
    ...
    async def post_async(self, shared, prep_res, exec_res):
        ...
        shared["mcp_client"] = multi_client
...
class GetToolsNode(AsyncNode):
    ...
    async def prep_async(self, shared):
        ...
        multi_client = shared.get("mcp_client")
        if not multi_client:
...
class DecideToolNode(AsyncNode):
    """Use LLM to analyze the question and decide which tool to use."""
    ...
    async def prep_async(self, shared):
        ...
        prompt = f"""
### CONTEXT
You are an AI assistant that can use tools via Model Context Protocol (MCP).
...
class ExecuteToolNode(AsyncNode):
    """Execute the chosen tool on the appropriate MCP server."""
...
async def run_multi_mcp_agent(question: str):
    """Run the multi-MCP agent with the given question."""
    
    # Create nodes
    init_node = InitializeMCPNode()
    ...
    execute_node - "success" >> cleanup_node
    no_tool_node - "cleanup" >> cleanup_node
...
def main():
    ...
    print("🚀 Multi-MCP Agent Starting")
    print(f"❓ Question: {question}")
    print("-" * 50)
    
    # Run the async agent
    try:
        asyncio.run(run_multi_mcp_agent(question))
